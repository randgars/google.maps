...
function findDistations() {
    for (var i = 0; i < locationList.length; i++) {
        var tempArr = Object.assign([], locationList);
        tempArr.splice(i, 1);
        service.getDistanceMatrix({
            origins: [locationList[i]],
        ...
        function callback(response, status) {
            if (status !== 'OK') {
                ...
                for (var i = 0; i < result.length; i++) {
                    pointDistancesArray.push({point: response.destinationAddresses[i], distance: result[i].distance.value, duration: result[i].duration.value});
                };
                pointsDistations = {
...
function findMinDistation() {
    debugger
    if (allDistances.length == locationList.length) {
            ...
            while (allDistances.length > 0) {
                ...
                tempDistances.sort(compareDistance);
                debugger
                var newPoint = tempDistances[0].distance;
