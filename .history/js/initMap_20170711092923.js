...
function initialize() {
    ...
    var onHandleChangeOrigin = function () {
        handleChangeOrigin(this);
    }

    originAutocomplete.addListener('place_changed', onHandleChangeOrigin);
    destinationAutocomplete.addListener('place_changed', onHandleChangeOrigin);
...
function handleChangeOrigin(value) {
    debugger
    geocoder.geocode({
        'address': originAutocomplete.getPlace().name
...
function handleChangeDestination() {
    geocoder.geocode({
        'address': destinationAutocomplete.getPlace().name
    }, function (results, status) {
        if (status == 'OK') {
            return new google.maps.Marker({
                position: results[0].geometry.location,
...
function addValueToList() {
    ...
    geocoder.geocode({
        'address': addWaypointsAutocomplete.getPlace().name
    }, function (results, status) {
        if (status == 'OK') {
