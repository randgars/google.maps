...
function calculateAndDisplayRoute(directionsService, directionsDisplay, map, service) {
    ...
    for (var i = 0; i < geocoderLocationList.length; i++) {
        ...
        function callback(response, status) {
            if (status !== 'OK') {
                ...
                debugger
                var childWaypoints = [];
                for (var i = 0; i < response.destinationAddresses[i].length; i++) {
                    debugger
                    childWaypoints.push({
                        pointAddress: response.destinationAddresses[i],
                        distance: response.rows[0].elements[i].distance.value
                    })
                }
                debugger
                var currentWaypoint = {
                    pointAddress: response.originAddresses[0], //current point
