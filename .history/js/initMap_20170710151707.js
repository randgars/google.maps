...
function testFunction(currentPoint, tempArr, geocoderLocationList) {
    var promise = new Promise(function (resolve, reject) {
            service.getDistanceMatrix({
                origins: [geocoderLocationList[currentPoint]],
...
function findDistations(directionsService, directionsDisplay, map, clickBtnValue) {
    ...
    //for (var i = 0; i < geocoderLocationList.length; i++) {        
        ...
        while (tempArr.length > 0) {
            ...
            tempArr.splice(currentPoint, 1);
            testFunction(currentPoint, tempArr, geocoderLocationList)
            .then(function (response) {
                debugger
                var sortDistance = response.rows[0].elements.sort(compareDistance);
                var minDistance = sortDistance[0]
                debugger
                ...
                for (var k = 0; k < result.length; k++) {
                    ...
                    tempArr = Object.assign([], geocoderLocationList);
                    tempArr.splice(k, 1);
                    pointDistancesArray.push({point: tempArr[k], distance: result[k].distance.value, name: response.destinationAddresses[k]});
                };
