...
function calculateMinDistance(routes) {
    ...
    minDistance = { distance: distanceValues[0].distance.value, duration: distanceValues[0].duration.value, index: 0 };

    for (var i = 0; i < distanceValues.length; i++) {
        if (distanceValues[i].distance.value < minDistance.distance) {
            minDistance = { distance: distanceValues[i].distance.value, duration: distanceValues[i].duration.value, index: i };
        }
    }
...
function calculateMinDuration(routes) {
    ...
    minDuration = { distance: durationValues[0].distance.value, duration: durationValues[0].duration.value, index: 0 };

    for (var i = 0; i < durationValues.length; i++) {
        debugger
        if (durationValues[i].duration.value < minDuration.duration) {
            minDuration = { distance: durationValues[i].distance.value, duration: durationValues[i].duration.value, index: i };
        }
    }
    debugger
