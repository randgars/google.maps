...
function findMinDistation(directionsService, directionsDisplay, parentWaypoints) {
    ...
    if (parentWaypoints.length == geocoderLocationList.length) {
        ...
        for (var i = 0, parts = [], max = 24; i < finalyWaypointsArray.length; i = i + max) {
            parts.push(finalyWaypointsArray.slice(i, i + max + 1));
        }
        directionsRequest(directionsService, directionsDisplay);
...
function directionsRequest(directionsService, directionsDisplay) {
    var totalDistance;
    for (var i = 0; i < parts.length; i++) {
        ...
        directionsService.route({
            ...
            if (status === 'OK') {
                calculateMinDistance(response.routes, totalDistance);
                directionsRendererFunction(response, minDistance);
            } else {
...
function calculateMinDistance(routes, totalDistance) {
    debugger
...
function directionsRendererFunction(response, routeIndex) {
